...
with open("faqs.json", "r", encoding="utf-8") as f:
    faqs = json.load(f) #reads and converts the file into a python object 

# Save updated faqs (Learning Mode)
def save_faqs():
    with open("faqs.json", "w", encoding="utf-8") as f:
        json.dump(faqs, f, indent=4) #storing the python object into a json file
...
def chatbot_response(user_input):
    ...
    if len(user_input.split()) >= 2:
        match = difflib.get_close_matches(user_input, questions, n=1, cutoff=0.50) #finds the strings (n amt) that are similar the entered string(based on a score bw 0 and 1)
        if match:
...
entry.focus() #puts the cursor the the entry box on opening 
